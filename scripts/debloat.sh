...
for json_file in "${json_files[@]}"; do
  ...
  for package_name in "${package_names[@]}"; do
    ...
    # checking whether the "removal" is either replace or delete. change it if you want.
    if [[ "$removal" == "delete" || "$removal" == "replace" ]]; then
      adb shell pm uninstall -k --user $(adb shell am get-current-user) "$package_name"
    else
